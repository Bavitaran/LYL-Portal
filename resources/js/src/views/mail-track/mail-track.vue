            ...
            <table class="table table-bordered">
              ...
              <tbody v-if="mails.length > 0">
                <tr class="text-center" v-for="(mail, key) in mails" :key="key">
                  ...
                  <td>
                                        <router-link :to='{name:"edit-mail",params:{id:mail.id}}' class="btn btn-success">Edit</router-link>
                                        <button type="button" @click="deleteMail(mail.id)" class="btn btn-danger">Delete</button>
                                    </td>
                </tr>
              </tbody>
              <tbody v-else>
  ...
  methods: {
    ...
    deleteMail(id){
            if(confirm("Are you sure to delete this record?")){
                this.axios.delete(`/api/auth/mail/${id}`).then(response=>{
                    this.getMails()
                }).catch(error=>{
                    console.log(error)
                })
        ...
        searchDate(){
            axios.get('/api/auth/mail?search_date=' + this.search_date)
            .then(response => this.mails = response.data)

        },
        searchRefNo(){
            axios.get('/api/auth/mail?search_ref_no=' + this.search_ref_no)
            .then(response => this.mails = response.data)

        },
        searchDescription(){
            axios.get('/api/auth/mail?search_description=' + this.search_description)
            .then(response => this.mails = response.data)
