                            ...
                            <tbody v-if="users.length > 0">
                                <tr v-for="(user,key) in users" :key="key">
                                    ...
                                    <td class="text-center">
                                        <router-link :to='{name:"apps-users-edit",params:{id:user.id}}' class="btn btn-success">Edit</router-link>
...
export default {
  ...
  methods:{
        async getUsers(){
            await this.axios.get('/api/auth/users').then(response=>{
        ...
        deleteUser(id){
            if(confirm("Are you sure to delete this user ?")){
                this.axios.delete(`/api/auth/users/${id}`).then(response=>{
                    this.getUsers()
                }).catch(error=>{
                    console.log(error)
                })
